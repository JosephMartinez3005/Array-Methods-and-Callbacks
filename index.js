...
console.log(fifaData.map((item) => {return item['Home Team Name']}))
...
function getFinals(array) {
    let finalStage = array.filter((item) => {return item.Stage === "Final"})
    return finalStage
 }
...
function getYears(arr, callback) {
    let years = callback(arr);
    return years.map((item) => {return item.Year})
}
